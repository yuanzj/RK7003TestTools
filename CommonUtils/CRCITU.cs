...
namespace CommonUtils
{
    ...
    public static partial class CRCITU
    {
        ...
        public static byte[] GetCrc16(Byte[] buffer)
        {
            int crc = 0;
            int l = buffer.Length;
            for (int i = 0; i < l; i++)
        ...
        /// 获取CRC字节数组
        /// </summary>
        /// <param name="crc">整形的CRC</param>
        /// <returns>两字节的CRC字节数组</returns>
        private static byte[] GetCRCBytes(int crc)
        {
            byte[] result = BitConverter.GetBytes(crc);
            Array.Resize(ref result, 2);                //只截取前面两个byte字节
        ...
        /// 检查给定长度数据的16位CRC是否正确  
        /// </summary>  
        /// <param name="FromHeadToVaild">要校验的字节数组</param>  
        /// <returns>  
        ///     true：正确  
        ///     false：错误  
        /// </returns>  
        ...
        /// 字节数组最后2个字节为校验码
        /// </reamrks>  
        public static Boolean IsCrc16Good(Byte[] FromHeadToCRC)
        {
        ...
        /// 获取客户端传来的包中的CRC数组
        /// </summary>
        /// <param name="FromHeadToCRC">从头到CRC校验域</param>
        /// <returns>CRC数组</returns>
        private static byte[] GetCRCFromPackage(byte[] FromHeadToCRC)
        {
            int iCRCLen = 2;                                                             //因为CRC占两个字节
            byte[] CRCValue = new byte[iCRCLen];                                          //用于存储包中CRC的值
            int i;
            for (i = 0; i < iCRCLen; i++)                                                 //得到一个临时字节数组    
            {
                CRCValue[i] = FromHeadToCRC[FromHeadToCRC.Length - iCRCLen + i];
            }
            return CRCValue;
        }
        /// <summary>
        /// 得到从头到有效数据的数据（不包含CRC验证域）
        /// </summary>
        /// <param name="FromHeadToCRC"></param>
        /// <returns></returns>
        private static byte[] GetHeadToVaildData(byte[] FromHeadToCRC)
        {
            int iLengh = FromHeadToCRC.Length - 2;               //因为CRC占两个字节
            byte[] FromHeadToVaild = new byte[iLengh];       //用于存储包头到有效数据域的bytes
            int i;
            for (i = 0; i < iLengh; i++)                     //得到一个临时字节数组    
            {
                FromHeadToVaild[i] = FromHeadToCRC[i];
            }
            return FromHeadToVaild;
