...
namespace RK7001Test
{
    ...
    public partial class MainForm : Form
    {
        ...
        public MainForm(string _comPort, int index)
        {
            ...
            mRK4110Pin = new Label[] { this.rkPin_1, this.rkPin_2, this.rkPin_3, this.rkPin_4, this.rkPin_5, this.rkPin_6, this.rkPin_7, this.rkPin_8, this.rkPin_9, this.rkPin_10,
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            mCoreTask.Update4103PinListHandler += (object _sender, EventArgs _e) =>
            {
                ...
                if (mArgs != null)
                {
                    SetRK4110PinList(mArgs.status, mArgs.level);
                }
            };
            ...
            SetRK4110PinList(null, INFO_LEVEL.INIT);
        ...
        private void StartTask()
        {
            ...
            SetRK4110ItemList(RK4110ITEM.INIT, null, INFO_LEVEL.INIT);
        ...
        #region 设置RK4110的列表项目
        delegate void SetRK4110ItemListCallback(RK4110ITEM item, DeviceInfo info, INFO_LEVEL level);
        private void SetRK4110ItemList(RK4110ITEM item, DeviceInfo info, INFO_LEVEL level)
        {
            if (this.lvRK4003ErrItem.InvokeRequired)//如果调用控件的线程和创建创建控件的线程不是同一个则为True
            {
                ...
                SetRK4110ItemListCallback d = new SetRK4110ItemListCallback(SetRK4110ItemList);
        ...
        #region 设置RK4110的PIN脚图
        private void SetRK4110PinList(PIN_STATUS status, INFO_LEVEL level)
        {
            if (level == INFO_LEVEL.INIT)
            {
                for (int i = 2; i <= 30; i++)
                {                    
                    if (i == 16 || i == 19 || i == 26 || i == 20 
                        ...
                        SetRK4110Pin(i, INFO_LEVEL.GREY);
                    else
                        SetRK4110Pin(i, INFO_LEVEL.INIT);
                }
                return;
            }
            else if (level == INFO_LEVEL.PROCESS)
            {
                ...
                {                    
                    if (i == 16 || i == 19 || i == 26 || i == 20 
                        ...
                        SetRK4110Pin(i, INFO_LEVEL.PROCESS);
                }
                return;
            }
            else if (level == INFO_LEVEL.PASS)
            {
                ...
                {                    
                    if (i == 16 || i == 19 || i == 26 || i == 20 
                        ...
                        SetRK4110Pin(i, INFO_LEVEL.PASS);
                }
                return;
            }
            ...
            /*
            if (status.Pin1_Open || status.Pin1_Short)
                SetRK4110Pin(1, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(1, INFO_LEVEL.PASS);
            */
            /*
            if (status.Pin2_Open || status.Pin2_Short)
                SetRK4110Pin(2, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(2, INFO_LEVEL.PASS);
            */
            if (status.Pin3_Open || status.Pin3_Short)
                SetRK4110Pin(3, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(3, INFO_LEVEL.PASS);
            if (status.Pin4_Open || status.Pin4_Short)
                SetRK4110Pin(4, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(4, INFO_LEVEL.PASS);
            /*
            if (status.Pin5_Open || status.Pin5_Short)
                SetRK4110Pin(5, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(5, INFO_LEVEL.PASS);
            */
            /*
            if (status.Pin6_Open || status.Pin6_Short)
                SetRK4110Pin(6, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(6, INFO_LEVEL.PASS);
            */
            if (status.Pin7_Open || status.Pin7_Short)
                SetRK4110Pin(7, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(7, INFO_LEVEL.PASS);
            if (status.Pin8_Open || status.Pin8_Short)
                SetRK4110Pin(8, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(8, INFO_LEVEL.PASS);
            
            if (status.Pin9_Open || status.Pin9_Short)
                SetRK4110Pin(9, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(9, INFO_LEVEL.PASS);
            
            if (status.Pin10_Open || status.Pin10_Short)
                SetRK4110Pin(10, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(10, INFO_LEVEL.PASS);
            if (status.Pin11_Open || status.Pin11_Short)
                SetRK4110Pin(11, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(11, INFO_LEVEL.PASS);
            if (status.Pin12_Open || status.Pin12_Short)
                SetRK4110Pin(12, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(12, INFO_LEVEL.PASS);
            if (status.Pin13_Open || status.Pin13_Short)
                SetRK4110Pin(13, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(13, INFO_LEVEL.PASS);
            if (status.Pin14_Open || status.Pin14_Short)
                SetRK4110Pin(14, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(14, INFO_LEVEL.PASS);            
            if (status.Pin15_Open || status.Pin15_Short)
                SetRK4110Pin(15, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(15, INFO_LEVEL.PASS);            
            if (status.Pin16_Open || status.Pin16_Short)
                SetRK4110Pin(16, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(16, INFO_LEVEL.PASS);
            
            if (status.Pin17_Open || status.Pin17_Short)
                SetRK4110Pin(17, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(17, INFO_LEVEL.PASS);
            
            if (status.Pin18_Open || status.Pin18_Short)
                SetRK4110Pin(18, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(18, INFO_LEVEL.PASS);
            /*
            if (status.Pin19_Open || status.Pin19_Short)
                SetRK4110Pin(19, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(19, INFO_LEVEL.PASS);
            */
            /*
            if (status.Pin20_Open || status.Pin20_Short)
                SetRK4110Pin(20, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(20, INFO_LEVEL.PASS);
            */
            if (status.Pin21_Open || status.Pin21_Short)
                SetRK4110Pin(21, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(21, INFO_LEVEL.PASS);
            if (status.Pin22_Open || status.Pin22_Short)
                SetRK4110Pin(22, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(22, INFO_LEVEL.PASS);
            if (status.Pin23_Open || status.Pin23_Short)
                SetRK4110Pin(23, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(23, INFO_LEVEL.PASS);
            if (status.Pin24_Open || status.Pin24_Short)
                SetRK4110Pin(24, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(24, INFO_LEVEL.PASS);
            if (status.Pin25_Open || status.Pin25_Short)
                SetRK4110Pin(25, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(25, INFO_LEVEL.PASS);
            /*
            if (status.Pin26_Open || status.Pin26_Short)
                SetRK4110Pin(26, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(26, INFO_LEVEL.PASS);
            */
            if (status.Pin27_Open || status.Pin27_Short)
                SetRK4110Pin(27, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(27, INFO_LEVEL.PASS);
            if (status.Pin28_Open || status.Pin28_Short)
                SetRK4110Pin(28, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(28, INFO_LEVEL.PASS);
            /*
            if (status.Pin29_Open || status.Pin29_Short)
                SetRK4110Pin(29, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(29, INFO_LEVEL.PASS);
            */
            /*
            if (status.Pin30_Open || status.Pin30_Short)
                SetRK4110Pin(30, INFO_LEVEL.FAIL);
            else
                SetRK4110Pin(30, INFO_LEVEL.PASS);
            */
        }
        #endregion

        #region 设置RK4110单个pin
        delegate void SetRK4110PinCallback(int num, INFO_LEVEL result);
        private void SetRK4110Pin(int num, INFO_LEVEL result)
        {
            if (mRK4110Pin[num - 1].InvokeRequired)//如果调用控件的线程和创建创建控件的线程不是同一个则为True
            {
                while (!mRK4110Pin[num - 1].IsHandleCreated)
                {
                    //解决窗体关闭时出现“访问已释放句柄“的异常
                    if (mRK4110Pin[num - 1].Disposing || mRK4110Pin[num - 1].IsDisposed)
                        return;
                }
                SetRK4110PinCallback d = new SetRK4110PinCallback(SetRK4110Pin);
                mRK4110Pin[num - 1].Invoke(d, new object[] { num, result });
            }
            else
            {
                switch (result)
                {
                    case INFO_LEVEL.FAIL:
                        mRK4110Pin[num - 1].BackColor = Color.Red;
                        break;
                    case INFO_LEVEL.PASS:
                        mRK4110Pin[num - 1].BackColor = Color.Green;
                        break;
                    case INFO_LEVEL.PROCESS:
                        mRK4110Pin[num - 1].BackColor = Color.Yellow;
                        break;
                    case INFO_LEVEL.INIT:
                        mRK4110Pin[num - 1].BackColor = Color.White;
                        break;
                    case INFO_LEVEL.GREY:
                        mRK4110Pin[num - 1].BackColor = Color.Gray;
                        break;
